...
public class MainActivity extends AppCompatActivity {
    ...
    private static final String DETAIL_PATH = "/DCIM/Camera/";

    // 카메라 광각, 전면, 후면
    private static final int CAM_WHAT = 2;
    private static final int CAM_FRONT = 1;
    private static final int CAM_REAR = 0;
    ...
    private TextureView.SurfaceTextureListener mSurfaceTextureListener = new TextureView.SurfaceTextureListener() {
        @Override
        public void onSurfaceTextureAvailable(SurfaceTexture surfaceTexture, int width, int height) { openCamera(width, height); }
        @Override
        public void onSurfaceTextureSizeChanged(SurfaceTexture surfaceTexture, int width, int height) { configureTransform(width, height); }
        @Override
    ...
    private static Size chooseVideoSize(Size[] choices) {
        for (Size size : choices) {
            if (size.getWidth() == size.getHeight() * 1.5 / 1 && size.getWidth() <= 720
            ) {
                return size;
            }
        }
            ...
            if (grantResults.length == VIDEO_PERMISSIONS.length) {
                for (int result : grantResults) {
                    if (result != PackageManager.PERMISSION_GRANTED) {
                        ErrorDialog.newInstance("This sample needs permission for camera and audio recording.").show(getFragmentManager(), FRAGMENT_DIALOG);
                        break;
                    }
                }
            } else {
    ...
    private void openCamera(int width, int height) {
        ...
        try {
            ...
            String cameraId = manager.getCameraIdList()[CAM_FRONT];
